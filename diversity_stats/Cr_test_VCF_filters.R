...
pl<-ggplot(STATS3, aes(x = classifiedWinEnd, y = STATS3[,stat], ordered = FALSE,color=Type))  +
  labs(title ="", x = "Genome position",y = colnames(STATS3)[stat]) +  scale_color_manual(values=filtcolors,name="Filter") +
...
tiff(paste0(colnames(STATS3)[stat],".filters.tiff"),width = 7.2,height = 3.3,res=300, units="in")
plot(pl)
dev.off()
...
image_write(combo, path = "S19.tiff", format = "tiff", quality = 100)
